...
while [ $b -gt 0 ]; do
    ...
    4) TOPLINE="$TOPLINE Je";;
    5) TOPLINE="$TOPLINE Ve";;
    6) TOPLINE="$TOPLINE Sa";;
    7) TOPLINE="$TOPLINE Di";;
    esac
    b=$[$b-1]
    y=$[$y+1]
    if [ $y -eq 8 ]
...
echo '${goto 1130}''${color gray}''${font mono:Bold:size=10}'$TOPLINE | sed 's/Di/${color red}Di${color gray}/g' | sed 's/Sa/${color red}Sa${color gray}/g'
echo '${goto 1130}''${font mono:bold:size=10}''${color gray}'$OVER '${color blue}''${font mono:bold:size=13}'$TODAY'${voffset -3}${color gray}''${font mono:bold:size=10}'$REST
